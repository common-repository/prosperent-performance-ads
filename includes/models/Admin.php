 ...
 * @package Model
 */
class Model_Ads_Admin extends Model_Ads_Base
{
	/**
	 ...
	 */
	public $adminPages = array('prosper_general', 'prosper_performAds', 'prosper_advanced');
	...
	{
		wp_register_style( 'prospere_admin_style', PROSPERADS_URL . 'includes/css/admin.min.css', array(), $this->getVersion() );
	...
	public function addActionLink( $links, $file ) 
	{
		...
		if ( empty( $this_plugin ) ) 
			$this_plugin = 'prosper-ads/prosperads.php';

		if ( $file == $this_plugin ) 
		{
			$settings_link = '<a href="' . admin_url( 'admin.php?page=prosperads_general' ) . '">' . __( 'Settings', 'prosperent_suite' ) . '</a>';
	...
	public function adminHeader( $title, $form = true, $option = 'prosperent_options', $optionshort = 'prosperSuite', $contains_files = false ) 
	{
		...
		?>
		<img src="<?php echo PROSPERADS_IMG . '/Gears-32.png'; ?>"/><?php echo '<h1 style="margin-left:8px;display:inline-block;font-size:34px;">ProsperAds</h1>';?>
